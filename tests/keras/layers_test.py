...
class Test_KerasLayers():
    ...
    def tests_compile_space(self):
        ...
        space.random_sample()
        assert dense.is_params_ready == True
        outputs = space.compile_space()
        assert len(outputs) == 1
        assert outputs[0].shape, (None, space.Module_Dense_2.param_values['units'])

        model = models.Model(inputs=space.Module_Input_1.output, outputs=outputs)
        model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        model.summary()
        assert model.get_layer('Module_Input_1').output.shape[1] == 3000
        assert model.get_layer('Module_Dense_1').output.shape[1] == space.Module_Dense_1.param_values['units']
